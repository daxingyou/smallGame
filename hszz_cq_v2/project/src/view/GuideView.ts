class GuideView extends BaseEuiView{
	...
	private setArrow(): void {
		...
		this.tipGroup.x = this.rect.x - 30;
		this.tipGroup.y = this.rect.y + (this.rect.height>>1);
		egret.Tween.removeTweens(this.tipGroup);
		egret.Tween.get(this.tipGroup,{loop:true}).to({x:this.tipGroup.x - 20},1000).to({x:this.tipGroup.x},1000);
	}
	public close():void{
		this.removeChildren();
		this._handMc = null;
