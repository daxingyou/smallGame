...
var GameMainView = (function (_super) {
    ...
    GameMainView.prototype.childrenCreated = function () {
    ...
    GameMainView.prototype.refreshLockCardList = function () {
        ...
        if (!ownCardlistVo.length) {
            ...
            ;
            var rightIndex_1 = this.curIndex + 1;
            if (rightIndex_1 > ownCardlistVo.length - 1) {
                rightIndex_1 = 0;
            }
            this.noCard = false;
            if (GameApp.guilding) {
                ...
                this.firstCard = this.createCard(ownCardlistVo[leftIndex], leftIndex, 0.7, { x: (this.cardGroup.width >> 1) - 180 }, null, function () {
                    _this.lastCard = _this.createCard(ownCardlistVo[rightIndex_1], rightIndex_1, 0.7, { x: (_this.cardGroup.width >> 1) + 180 }, null, function () {
                        _this.centerCard = _this.createCard(ownCardlistVo[_this.curIndex], _this.curIndex, 1, { x: (_this.cardGroup.width >> 1) }, null, function () {
                            //执行引导
                            _this.showDesc();
                            ViewManager.inst().open(GuideView);
                            var guildView = ViewManager.inst().getView(GuideView);
                            guildView.nextStep({ id: "1_1", comObj: _this.battleBtn, width: 107, height: 115 });
                        }, _this);
                    }, _this);
                }, this);
            }
                ...
                this.lastCard = this.createCard(ownCardlistVo[rightIndex_1], rightIndex_1, 0.7, { x: (this.cardGroup.width >> 1) + 180 });
                this.centerCard = this.createCard(ownCardlistVo[this.curIndex], this.curIndex, 1, { x: (this.cardGroup.width >> 1) });
            }
            this.showDesc();
        }
    };
    GameMainView.prototype.createCard = function (cardVo, index, scale, attr, alpha, cb, thisarg) {
        ...
        this.setCardIndex();
        if (!!cardVo) {
            var ligntColor = [0x76ff39, 0x3ecfff, 0xbf11ff, 0xffd800];
            GlobalFun.lighting(curcard, ligntColor[cardVo.quality - 1]);
            if (GameApp.guilding) {
                curcard.alpha = 0;
                curcard.scaleX = curcard.scaleY = 0;
                egret.Tween.get(curcard).to({ alpha: 1, scaleX: scale, scaleY: scale }, 1000, egret.Ease.bounceOut).call(function () {
