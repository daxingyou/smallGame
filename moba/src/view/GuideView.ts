class GuideView extends BaseEuiView{
	...
	/**id值 引导的对象 宽 高 */
	private data:{ "id": string, "comObj": any, width: number, height: number ,offsetX?:number,offsetY?:number,cnt?:string};
	...
	private onGuideTap(evt:egret.TouchEvent):void{
		...
		if(!itemCfg.next){
			ViewManager.single<ViewManager>().close(GuideView);
		}
	}
	//执行下一步
	public nextStep(data:{ "id": string, "comObj": any, width: number, height: number ,offsetX?:number,offsetY?:number,cnt?:string}):void{
		this.data = data;
		this.setRect();
	}
	...
	private setArrow(): void {
		...
		if(this.data.cnt){
			...
			this.tipTxt.textFlow = new egret.HtmlTextParser().parse(this.data.cnt);
		}else{
			this.tipTxt.visible = false;
		}
		this.tipTxt.anchorOffsetX = this.tipTxt.width>>1;
		this.tipTxt.anchorOffsetY = this.tipTxt.height;
		this.tipTxt.x = this.rect.x + (this.rect.width>>1);
		this.tipTxt.y = this.rect.y - 20;
	}
	public close():void{
		this.removeChildren();
		this._handMc = null;
