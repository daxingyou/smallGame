...
class MovieClip extends egret.MovieClip {
	...
	public constructor(bounce:boolean = false) {
		super();
	...
	/**播放序列帧中的切片合集 */
	public playSlice(name:string,framelabel:string,action:string,playTime:number = -1,frameRate?:number):void{
		var data = RES.getRes(`${name}_json`);
        var txtr = RES.getRes(`${name}_png`);
        var mcFactory:egret.MovieClipDataFactory = new egret.MovieClipDataFactory( data, txtr );
		this.movieClipData = mcFactory.generateMovieClipData( framelabel )
        // var mc1:egret.MovieClip = new egret.MovieClip( );
		this.frameRate = frameRate?frameRate:12;
        this.gotoAndPlay(action,playTime)
	}
	/**
	 * 创建主体动画
	 */
