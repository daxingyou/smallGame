...
class MovieClip extends egret.MovieClip {
	...
	public changeActionFrameLab(str:string,playCount?:number):void{
		if(this.changeStr == str){return}
		this.changeStr = str;
		this.gotoAndPlay(str, playCount?playCount:this.playCount);
		if(playCount && playCount > 0){
			let time:number = playCount*600;
			let self = this;
			let timeout = setTimeout(function() {
				clearTimeout(timeout);
				self.changeActionFrameLab("stand");
			}, time);
		}
	}
	/**
