class SoldierEntity extends BaseEntity{
	...
	public attrCfg:{name:string,level:number,hp:number,atk:number,type:number,curCapacity:number,capacity:number,insId:number}
	...
	public setSoldierData(camp:number,res:string,id:number,_state?:number):void{
		this._camp = camp;
		...
		if(this.general){
			...
			levelLab.size = 12;
			let level:number = camp == 1?this.attrCfg.level + 1:this.attrCfg.level;
			...
			if(this.curState != ActionState.ATTACK){
				...
				let timeout = setTimeout(function() {
					...
					if(self && self._mc){
						...
						self._mc.playFile(self._res,-1,null,false,ActionState.RUN);
					}
