class OutWildBattle extends BaseEuiView{
	...
	private activeWeaponNum:number = 0;
	...
	private onItemTap(evt:eui.ItemTapEvent):void{
		...
		to({x:stagep.x,y:stagep.y,scaleX:0,scaleY:0,alpha:0},300,egret.Ease.circIn).call(()=>{
		...
		switch(this.curItem.cardVo.cardType){
			...
			case 1:
				...
				GlobalFun.createSkillEff(1,skillres[index],LayerManager.MAP_LAYER,1,{x:MapView.inst().roles[0].x,y:MapView.inst().roles[0].y});
