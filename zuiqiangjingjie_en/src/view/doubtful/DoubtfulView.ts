class DoubtfulView extends BaseEuiView
{
	...
	private formation:number = 3; /**Number of square matrices */
	...
	private touchTap(evt:egret.TouchEvent)
	{
		switch(evt.target)
		{
			...
			case this.all_btn:
				...
				this.switchBtn(this.all_btn);
				break;
			case this.check: /**View enemy lineup */
				if(this.checkBool)
				{
					UserTips.inst().showTips("Cannot be viewed repeatedly！");
					return;
				}
				...
				{
					UserTips.inst().showTips("Insufficient intelligence value");
				}
				break;
			case this.start_game: /**Start button */
	...
	private createWj(id:number, group:eui.Group, group_id:number)
	{
		for(let i = 0; i < GameApp.ownSolderis.length; i++)
		{
			...
			{
				UserTips.inst().showTips("General can't reuse!");
				return;
			}
		}
		GameApp.soldiersNum++;
	...
	/**Create soldiers _formationNumber of square matrices _idArmsid _groupContainer created _anyCharacter set*/
	...
	private createMoveSoldier(evt:CustomEvt)
	{
		...
		if(card.soldierType == 1 || card.soldierType == 2)
		{
			if(card.ownNum < 36)
			{
				UserTips.inst().showTips("Insufficient Corps！");
				return;
			}
	...
	private update()
	{
		...
		this.qingbao.text = "intelligence:" + GameApp.intelligence;

		this.fighting_p.text = "" + GameCfg.playerAttack;
		this.fighting_n.text = "" + GameCfg.npcAttack;
		this.soldiers.text = "army:" + GameApp.soldiersNum;
		if(this.wj0 == null)
		{
