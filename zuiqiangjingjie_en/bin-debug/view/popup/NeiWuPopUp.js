...
var NeiWuPopUp = (function (_super) {
    ...
    NeiWuPopUp.prototype.open = function () {
        ...
        if (!cdTimestring || (cdTimestring && parseInt(cdTimestring) <= new Date().getTime())) {
            //There is no housekeeping at present Or the interior cold zone is over
    ...
    NeiWuPopUp.prototype.judageReward = function (option) {
        ...
        if (option) {
            //The current option has a value
            for (var key in option) {
                if (key == "goods") {
                    GameApp.goods += option.goods;
                    UserTips.inst().showTips("Get suppliesx" + option.goods);
                }
                else if (key == "medal") {
                    GameApp.medal += option.medal;
                    UserTips.inst().showTips("Medal of honorx" + option.medal);
                }
                else if (key == "general") {
                    ...
                    UserTips.inst().showTips("Get the general" + rewardGeneral.name + "x1");
                }
            }
        }
                    ...
                    if (i >= _this.items.length - 1) {
                        ...
                        var timeout_1 = setTimeout(function () {
                            ...
                            if (!countstr || (countstr && parseInt(countstr) < 3)) {
                                //The current times can also handle internal affairs
                                this.storyStep = 1;
                                self_1.showStory();
                            }
                            else {
                                //The internal affairs currently handled are greater than or equal to3second
