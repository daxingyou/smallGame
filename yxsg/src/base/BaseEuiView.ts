abstract class BaseEuiView extends eui.Component implements IBaseEuiView{
	 ...
	 * 添加主ui控件 头像组件,底部经验条
	 */
	public addMainCom(param?:any,bagBoo:boolean = true,parent:egret.DisplayObjectContainer = null,hideHead:boolean = false,offestY:number = 0):void{
		if(!hideHead){
		...
		if(bagBoo){
			...
			bagimg.right = 90;
			bagimg.top = -143;
			egret.Tween.get(bagimg).to({top:10 + offestY},600,egret.Ease.circOut).call(()=>{
			...
			shopImg.right = 11;
			shopImg.top = -143;
			egret.Tween.get(shopImg).to({top:10+ offestY},600,egret.Ease.circOut).call(()=>{
