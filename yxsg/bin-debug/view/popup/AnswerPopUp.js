...
var AnswerPopUp = (function (_super) {
    ...
    AnswerPopUp.prototype.open = function () {
        ...
        var timespan = egret.localStorage.getItem(LocalStorageEnum.ANSWER_TIMESPAN);
        if (timespan) {
        ...
        var answerCount = egret.localStorage.getItem(LocalStorageEnum.ANSWER_COUNT);
    ...
    AnswerPopUp.prototype.onStart = function () {
        ...
        if (!this._ifWild) {
            ...
            if (answerCount && parseInt(answerCount) >= this.answerCount) {
                UserTips.ins().showTips("本日答题完毕");
                return;
            }
            if (timespan) {
                var nowTimespan = new Date().getTime();
                if (nowTimespan - parseInt(timespan) < 10 * 60 * 1000) {
                    UserTips.ins().showTips("\u7B54\u9898\u51B7\u5374\u4E2D\u3002\u3002\u3002\u9884\u8BA1\u5728<font color='0x00ff00'>" + DateUtils.getFormatBySecond((parseInt(timespan) + 10 * 60 * 1000) / 1000, DateUtils.TIME_FORMAT_14) + "</font>\u89E3\u9501");
                    return;
                }
            }
