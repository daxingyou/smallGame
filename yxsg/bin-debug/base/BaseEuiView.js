...
var BaseEuiView = (function (_super) {
     ...
     * 添加主ui控件 头像组件,底部经验条
     */
    BaseEuiView.prototype.addMainCom = function (param, bagBoo, parent, hideHead, offestY) {
        if (bagBoo === void 0) { bagBoo = true; }
        if (parent === void 0) { parent = null; }
        if (hideHead === void 0) { hideHead = false; }
        if (offestY === void 0) { offestY = 0; }
        if (!hideHead) {
            var roleCom_1 = new RoleHeadCom();
            this.roleHeadCom = roleCom_1;
            if (parent) {
                parent.addChild(roleCom_1);
            }
            else {
                this.addChildAt(roleCom_1, 1);
            }
            roleCom_1.left = 10;
            roleCom_1.top = -100;
            if (param && param.length) {
                roleCom_1.initialize({ roleName: param[0].roleName, goldNum: "0", levelName: "", headIcon: param[0].headIcon });
            }
            egret.Tween.get(roleCom_1).to({ top: 20 }, 600, egret.Ease.circOut).call(function () {
                egret.Tween.removeTweens(roleCom_1);
            }, this);
        }
        ...
        if (bagBoo) {
            ...
            bagimg_1.right = 90;
            bagimg_1.top = -143;
            egret.Tween.get(bagimg_1).to({ top: 10 + offestY }, 600, egret.Ease.circOut).call(function () {
            ...
            shopImg_1.right = 11;
            shopImg_1.top = -143;
            egret.Tween.get(shopImg_1).to({ top: 10 + offestY }, 600, egret.Ease.circOut).call(function () {
