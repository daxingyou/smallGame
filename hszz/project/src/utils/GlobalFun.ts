    ...
    public static refreshCardData(attr:CardVo):void{
		...
		for(let key  in cardData){
            ...
            if(cardData[key].ownNum > 0){
                cardData[key].ifUnlock = true;
            }
			if(cardData[key].ifUnlock){
				arr.push(cardData[key])
			}else{
				unlockArr.push(cardData[key]);
			}
		}
		GameApp.unlocks = unlockArr;
		GameApp.locks = arr;
        egret.localStorage.setItem(LocalStorageEnum.ROLE_OWNER_CARDIDS,JSON.stringify(cardData));
