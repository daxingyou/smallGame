...
var BattleView = (function (_super) {
    ...
    BattleView.prototype.open = function () {
        ...
        egret.Tween.get(this.topGroup).to({ top: 0 }, 600, egret.Ease.circOut).call(function () {
            egret.Tween.removeTweens(_this.topGroup);
        }, this);
        egret.Tween.get(this.shop_btn).to({ bottom: 11 }, 600, egret.Ease.circOut).call(function () {
            egret.Tween.removeTweens(_this.shop_btn);
        }, this);
        egret.Tween.get(this.back_btn).to({ right: 44 }, 600, egret.Ease.circOut).call(function () {
            egret.Tween.removeTweens(_this.back_btn);
        }, this);
        egret.Tween.get(this.skillGroup).to({ bottom: 4 }, 600, egret.Ease.circOut).call(function () {
        ...
        this.player_exp.text = "经验：" + GameApp.exp;
        ...
        this.npc_exp.text = "经验：" + Math.floor(Math.random() * 300 + 500);
        ...
        this.enemyHpCom.initData("flag_" + index + "_png", cityName ? cityName + "统领" : "山贼");
            ...
            var _loop_2 = function (j) {
                var timeout2 = setTimeout(function () {
                    ...
                    var _loop_3 = function (i) {
                        ...
                        var bullet = new Bullet(arrowArr[i].x + 50, arrowArr[i].y, randomX, arrowArr[i].y, 0, "game_arrow0_png", function () {
            ...
            var _loop_6 = function (j) {
                var timeout2 = setTimeout(function () {
                    ...
                    var _loop_7 = function (i) {
                        ...
                        bullet.x = self.enemyGroup.x - Math.random() * 300;
                        bullet.y = -80;
                        var scale = Math.random() * 0.5 + 0.5;
                        bullet.scaleX = bullet.scaleY = scale;
                        bullet.rotation = Math.random() * 360;
                        egret.Tween.get(bullet)
                            .to({ x: randomX, y: arrowArr[i].y + 80 }, 800, egret.Ease.sineIn)
                            .call(function () {
                            var mc = new MovieClip();
    ...
    BattleView.prototype.touchEnd = function (evt) {
        ...
        if (Math.floor(this.moveCard.id / 10) == 10010 || Math.floor(this.moveCard.id / 10) == 1000) {
            ...
            // 	}else
            // 	{
            // 		UserTips.inst().showTips("不在使用范围区域");
            // 	}
            // }
        }
        else {
