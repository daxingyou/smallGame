class BattleView extends BaseEuiView{
	...
	public open(...param):void{
		...
		egret.Tween.get(this.topGroup).to({top:0},600,egret.Ease.circOut).call(()=>{
			egret.Tween.removeTweens(this.topGroup);
		},this)
		egret.Tween.get(this.shop_btn).to({bottom:11},600,egret.Ease.circOut).call(()=>{
			egret.Tween.removeTweens(this.shop_btn);
		},this)

		egret.Tween.get(this.back_btn).to({right:44},600,egret.Ease.circOut).call(()=>{
			egret.Tween.removeTweens(this.back_btn);
		},this)

		egret.Tween.get(this.skillGroup).to({bottom:4},600,egret.Ease.circOut).call(()=>{
			...
			for(let j:number = 0;j<20;j++){
				let timeout2 = setTimeout(function() {
					...
					for(let i:number = 0;i< arrowArr.length;i++){
						...
						let bullet:Bullet = new Bullet(arrowArr[i].x + 50, arrowArr[i].y, randomX, arrowArr[i].y, 0, "game_arrow0_png",()=>{
