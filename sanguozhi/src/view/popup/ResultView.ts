class ResultView extends BaseEuiView{
	...
	public open(...param):void{
		...
		if(param[0].state == "win"){
			...
			let generals:CardAttrVo[] = GlobalFun.getCardsFromType(CardType.general,false);
			...
			for(let i:number = 0;i<numIndex;i++){
				let cardIndex:number = ( Math.random()*generals.length)>>0;
				let cardAttr:CardAttrVo = generals[cardIndex];
				if(numIndex == 1){
					this.card1.visible = true;
					this.card1.initData(cardAttr,false);
				}else if(numIndex == 2){
					this.card2.visible = this.card3.visible = true;
					!!i?this.card3.initData(cardAttr,false):this.card2.initData(cardAttr,false);
				}else{
